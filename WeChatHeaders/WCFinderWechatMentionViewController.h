//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MMUIViewController.h"

#import "UITableViewDataSource-Protocol.h"
#import "UITableViewDelegate-Protocol.h"
#import "WCFinderFansProfileViewControllerDelegate-Protocol.h"
#import "WCFinderFeedBaseViewControllerProtocol-Protocol.h"
#import "WCFinderRefreshTableFooterViewDelegate-Protocol.h"
#import "WCFinderWechatMentionCellDelegate-Protocol.h"
#import "WCFinderWechatMentionViewModelDelegate-Protocol.h"
#import "WCGeneralMonitorDelegate-Protocol.h"

@class NSString, UILabel, UITableView, WCFinderRefreshTableFooterView, WCFinderWechatMentionCellViewModel, WCFinderWechatMentionViewModel;

@interface WCFinderWechatMentionViewController : MMUIViewController <WCFinderWechatMentionViewModelDelegate, UITableViewDelegate, UITableViewDataSource, WCFinderRefreshTableFooterViewDelegate, WCFinderWechatMentionCellDelegate, WCFinderFeedBaseViewControllerProtocol, WCFinderFansProfileViewControllerDelegate, WCGeneralMonitorDelegate>
{
    _Bool _isFetchMore;
    WCFinderWechatMentionViewModel *_viewModel;
    UITableView *_tableView;
    WCFinderRefreshTableFooterView *_footerView;
    UILabel *_emptyTipsView;
    long long _unreadLikeCount;
    long long _unreadCommentCount;
    long long _unreadNotifyCount;
    double _enterTime;
    WCFinderWechatMentionCellViewModel *_showProfileViewModel;
}

- (void).cxx_destruct;
@property(retain, nonatomic) WCFinderWechatMentionCellViewModel *showProfileViewModel; // @synthesize showProfileViewModel=_showProfileViewModel;
@property(nonatomic) double enterTime; // @synthesize enterTime=_enterTime;
@property(nonatomic) long long unreadNotifyCount; // @synthesize unreadNotifyCount=_unreadNotifyCount;
@property(nonatomic) long long unreadCommentCount; // @synthesize unreadCommentCount=_unreadCommentCount;
@property(nonatomic) long long unreadLikeCount; // @synthesize unreadLikeCount=_unreadLikeCount;
@property(nonatomic) __weak UILabel *emptyTipsView; // @synthesize emptyTipsView=_emptyTipsView;
@property(nonatomic) _Bool isFetchMore; // @synthesize isFetchMore=_isFetchMore;
@property(retain, nonatomic) WCFinderRefreshTableFooterView *footerView; // @synthesize footerView=_footerView;
@property(nonatomic) __weak UITableView *tableView; // @synthesize tableView=_tableView;
@property(retain, nonatomic) WCFinderWechatMentionViewModel *viewModel; // @synthesize viewModel=_viewModel;
- (void)registerYReportSdk;
- (void)monitorView:(id)arg1 newExposedItems:(id)arg2 lastExposedItems:(id)arg3;
- (void)fansProfileViewController:(id)arg1 clickFinderActionWithWXUsername:(id)arg2;
- (void)fansProfileViewController:(id)arg1 clickPrivateMsgWithContact:(id)arg2 finderContact:(id)arg3 comment:(id)arg4;
- (unsigned long long)feedViewerScene;
- (unsigned long long)feedViewControllerScene;
- (void)finderFooterDidTriggerRefresh:(id)arg1;
- (void)wechatMentionDeleteIndexPath:(id)arg1;
- (void)wechatMentionUpdateSuccessful;
- (void)wechatMentionUpdateFail;
- (void)reloadNoMoreStatus;
- (void)wechatMentionNoMoreData;
- (void)wechatMentionNeedReloadData;
- (void)fetchNewestMentions;
- (void)showDetailDataItemVC:(id)arg1 autoShowListType:(unsigned long long)arg2 fromFirstLike:(_Bool)arg3;
- (void)showCommonWarningTipsWithContent:(id)arg1;
- (void)showDataItemWasPrivateSetTips;
- (void)showDataItemWasDeletedTips;
- (void)showCantSupportFanProfileTips;
- (void)showProfileWithViewModel:(id)arg1;
- (void)openDataItem:(id)arg1 showCommentList:(_Bool)arg2 refCommentID:(unsigned long long)arg3 showFriendLike:(_Bool)arg4 refFriendUsername:(id)arg5 fromFirstLike:(_Bool)arg6;
- (void)jumpToDataItemWithViewModel:(id)arg1;
- (void)wechatMentionCell:(id)arg1 didClickAvatarWithViewModel:(id)arg2;
- (_Bool)tableView:(id)arg1 canEditRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 trailingSwipeActionsConfigurationForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForHeaderInSection:(long long)arg2;
- (id)tableView:(id)arg1 viewForHeaderInSection:(long long)arg2;
- (id)createRetryView;
- (id)createLoadingView;
- (double)tableView:(id)arg1 heightForFooterInSection:(long long)arg2;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (void)scrollViewDidScroll:(id)arg1;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (void)configureTableView;
- (void)viewDidBePoped:(_Bool)arg1;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillLayoutSubviews;
- (void)viewDidLoad;
- (id)initWithType:(unsigned long long)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

