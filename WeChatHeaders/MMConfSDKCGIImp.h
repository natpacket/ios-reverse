//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "PBMessageObserverDelegate-Protocol.h"

@class NSString;

@interface MMConfSDKCGIImp : NSObject <PBMessageObserverDelegate>
{
    NSString *_currentRequestAppId;
    CDUnknownBlockType _currentGetAuthCodeCGICallback;
    CDUnknownBlockType _currentGetSDKUserInfoCGICallback;
    CDUnknownBlockType _currentGetUserInfoCGICallback;
    CDUnknownBlockType _currentInviteCGICallback;
    CDUnknownBlockType _currentSendBannerMsgCGICallback;
    CDUnknownBlockType _currentCreateCGICallback;
    CDUnknownBlockType _currentJoinCGICallback;
    CDUnknownBlockType _currentAddMemberCGICallback;
    CDUnknownBlockType _currentCancelInviteCGICallback;
}

- (void).cxx_destruct;
- (void)MessageReturn:(id)arg1 Event:(unsigned int)arg2;
- (void)cgiSendCloudCancelRequest:(unsigned int)arg1 data:(id)arg2;
- (void)cgiSendCloudAckRequest:(unsigned int)arg1 data:(id)arg2;
- (void)cgiSendCloudInviteRequest:(unsigned int)arg1 data:(id)arg2;
- (void)sendRequest:(id)arg1 Retry:(int)arg2;
- (void)cgiReportLog:(id)arg1 sdkGroupId:(id)arg2 fromUserName:(id)arg3 fromSdkUserName:(id)arg4 reportList:(id)arg5;
- (void)cgiSendBannerMsg:(unsigned int)arg1 groupid:(id)arg2 roomId:(long long)arg3 fromUserName:(id)arg4 fromSdkUserName:(id)arg5 bannerMsg:(id)arg6 callback:(CDUnknownBlockType)arg7;
- (void)cgiCancelInvite:(unsigned int)arg1 groupId:(id)arg2 roomId:(long long)arg3 sdkGroupId:(id)arg4 fromUserName:(id)arg5 fromSdkUserName:(id)arg6 toUserList:(id)arg7 callback:(CDUnknownBlockType)arg8;
- (void)cgiInvite:(unsigned int)arg1 groupId:(id)arg2 roomId:(long long)arg3 sdkGroupId:(id)arg4 fromUserName:(id)arg5 fromSdkUserName:(id)arg6 toUserList:(id)arg7 inviteItems:(id)arg8 callback:(CDUnknownBlockType)arg9;
- (void)cgiAddMember:(unsigned int)arg1 wxGroupId:(id)arg2 sdkGroupId:(id)arg3 calleeUserNameList:(id)arg4 callback:(CDUnknownBlockType)arg5;
- (void)cgiJoin:(unsigned int)arg1 wxGroupId:(id)arg2 sdkGroupId:(id)arg3 callback:(CDUnknownBlockType)arg4;
- (void)cgiCreate:(unsigned int)arg1 wxGroupId:(id)arg2 calleeUserNameList:(id)arg3 callback:(CDUnknownBlockType)arg4;
- (void)cgiGetUserInfo:(unsigned int)arg1 userList:(id)arg2 callback:(CDUnknownBlockType)arg3;
- (void)cgiGetSDKUserInfo:(unsigned int)arg1 sdkUserList:(id)arg2 groupId:(id)arg3 callback:(CDUnknownBlockType)arg4;
- (void)cgiGetAuthCode:(unsigned int)arg1 callback:(CDUnknownBlockType)arg2;

@end

